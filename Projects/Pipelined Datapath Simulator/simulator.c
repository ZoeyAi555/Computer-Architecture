...
int main(int argc, char *argv[]) {
    ...
    while (opcode(state.MEMWB.instr) != HALT) {
        ...
        //lw dependency (prev ID is LW)
        if(opcode(state.IDEX.instr) == LW){
            if(opcode(newState.IDEX.instr) == LW){
                if(field0(newState.IDEX.instr) == field1(state.IDEX.instr)){
        ...
        if(opcode(state.WBEND.instr) == LW){
            if (opcode(newState.EXMEM.instr)!=NOOP){
                ...
                if (opcode(newState.EXMEM.instr)!=LW && field1(newState.EXMEM.instr)==field1(state.WBEND.instr))
        ...
        if(opcode(state.WBEND.instr)== ADD ||opcode(state.WBEND.instr)== NOR){
            if (opcode(newState.EXMEM.instr)!=NOOP){
                if (field0(newState.EXMEM.instr)==field2(state.WBEND.instr))
                    regA = state.WBEND.writeData;
                if (opcode(newState.EXMEM.instr)!=LW && field1(newState.EXMEM.instr)==field2(state.WBEND.instr))
                    regB = state.WBEND.writeData;
            }
        }
        ...
        if (opcode(state.MEMWB.instr)==LW) {
            if (opcode(newState.EXMEM.instr)!=NOOP){
                ...
                if (opcode(newState.EXMEM.instr)!=LW && field1(newState.EXMEM.instr)==field1(state.MEMWB.instr)){
        ...
        if(opcode(state.MEMWB.instr)== ADD ||opcode(state.MEMWB.instr)== NOR){
            if (opcode(newState.EXMEM.instr)!=NOOP){
                if (field0(newState.EXMEM.instr)==field2(state.MEMWB.instr))
                    regA = state.MEMWB.writeData;
                if (opcode(newState.EXMEM.instr)!=LW && field1(newState.EXMEM.instr)==field2(state.MEMWB.instr))
                    regB = state.MEMWB.writeData;
            }
        }   
        ...
        if(opcode(state.EXMEM.instr)==LW){
            if (opcode(newState.EXMEM.instr)!=NOOP){
                ...
                if (opcode(newState.EXMEM.instr)!=LW && field1(newState.EXMEM.instr)==field1(state.EXMEM.instr)){
        ...
        if(opcode(state.EXMEM.instr)== ADD ||opcode(state.EXMEM.instr)== NOR){
            if (opcode(newState.EXMEM.instr)!=NOOP){
                if (field0(newState.EXMEM.instr)==field2(state.EXMEM.instr))
                    regA = state.EXMEM.aluResult;
                if (opcode(newState.EXMEM.instr)!=LW && field1(newState.EXMEM.instr)==field2(state.EXMEM.instr))
                    regB = state.EXMEM.aluResult;
            }
        }
        if(opcode(newState.EXMEM.instr) != NOOP){
        ...
        else if(opcode(newState.MEMWB.instr) != NOOP){
